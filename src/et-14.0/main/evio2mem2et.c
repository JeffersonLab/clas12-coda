/* evio2mem2et.c reads evio file and injects events into ET system */
...
static uint32_t *buffer, *bufptr;

#define MAXBUF 1100000
static int maxbufbytes = MAXBUF*4;;
static unsigned int buf[MAXBUF];

#define ER_BUF_SIZE 2147000000
...
main(int argc,char **argv)
{
  ...
  int             iterations=1, count, evcount, evtotal, nevents, nwtotal;
  int64_t         nbtotal;
  double          datacount;
  et_att_id	  attach1;
  et_sys_id       id;
  ...
  deltatime.tv_sec  = 0;
  deltatime.tv_nsec = 1000000L; /* 1 millisec */
  ...
  /* allocate buffer to store up to 2GB data file */
  buffer = malloc(ER_BUF_SIZE);
  if(buffer>0)
  {
  ...
  {
	printf("ERROR: cannot open >%s<, evOpen status %d - exit\n",filename,status);
    fflush(stdout);
    exit(0);
  }
...
next_event:
  ...
  {
    /*copy event into the buffer*/

    buflen = buf[0]+1;
    /*printf("[%d] buflen=%d\n",evtotal,buflen);fflush(stdout);*/

    /* check buffer overflow */
    if( (nbtotal+(buflen<<2)) >= ER_BUF_SIZE )
	{
      printf("End of buffer reached, close file\n");
      status = evClose(handle);
      goto out;
	}
	...
	*/
    ...
    for(i=0; i<buflen; i++) *bufptr++ = buf[i];
    evtotal ++;
	/*if(evtotal>150) exit(0);*/
    nwtotal += buflen;
    nbtotal = nwtotal<<2;
    /*printf("Ev %d, nwtotal=%d words (%lld bytes)\n",evtotal,nwtotal,nbtotal);*/
...
out:
  ...
  printf("Got %d events from file and placed them into the memory buffer\n",evtotal);
...
restartLinux:
  ...
  while (et_alive(id))
  {
    ...
    for (j=0; j<NUMLOOPS; j++)
    {
	  ...
	  /*status = et_events_new(id, attach1, pe, ET_TIMED, &deltatime, event_size, CHUNK, &count);*/
      if(status != ET_OK) printf("status=%d\n",status);

      if (status == ET_OK)
      {
        /*printf("Got %d of new events from ET system\n",count);fflush(stdout)*/;
      }
      ...
      for(i=0; i<count; i++)
      {
		...
		/*printf("Got from memory buffer event %d, len=%d words\n",evcount,buflen);fflush(stdout);*/
		...
		/* slow
		for(ii=0; ii<buflen; ii++) buf[ii] = *bufptr++;
        status = evWrite(handle1, buf);
        */

		
        status = evWrite(handle1, bufptr);
        bufptr += buflen;
        ...
        if(status!=0) printf("evWrite returns %d (0x%08x) evcount=%d buflen=%d\n",
                              status,status,evcount,buflen);

        evGetBufferLength(handle1,&len);
		/*printf("len=%d buflen=%d\n",len,buflen);fflush(stdout);*/
