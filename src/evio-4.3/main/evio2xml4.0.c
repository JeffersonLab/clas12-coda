...
dump_composite(unsigned int *buf)
{
  ...
  /* get format info from tagfragment */
  length  	= (buf[0]&0xffff)+1;
  type    	= (buf[0]>>16)&0xf;
  tag     	= (buf[0]>>20)&0xfff;
  num     	= -1;   /* doesn't have num */
  ...
  */
...

  {
	...
	/*sergey: can use strlen instead of tag ..
	printf("+++++++++++++++++++++++++++++++++ tag=%d len=%d\n",tag,strlen(ch));
	*/
