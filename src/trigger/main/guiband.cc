...
// guiband.cxx: gui for BAND v1495-based trigger
...
GUIMainFrame::GUIMainFrame(const TGWindow *p, UInt_t w, UInt_t h, char *host) : TGMainFrame(p, w, h)
{
   ...
   fMenuDock->SetWindowName("GUIBAND Menu"); // ???
   ...
   // menu item 'BAND'
   fMenuBAND = new TGPopupMenu(fClient->GetRoot());
   fMenuBAND->AddLabel("BAND monitoring and control");
   fMenuBAND->AddSeparator();
   fMenuBAND->AddEntry("&Registers", M_REGISTERS);
   fMenuBAND->AddEntry("&Delays", M_DELAYS);
   fMenuBAND->AddEntry("&Scalers", M_SCALERS);
   fMenuBAND->AddEntry("&Dsc2", M_DSC2);
   fMenuBAND->AddEntry("&Scope (ASCII)", M_SCOPE_ASCII);
   fMenuBAND->AddEntry("&Scope (Canvas)", M_SCOPE_CANVAS);
   ...
   fMenuBar->AddPopup("&BAND", fMenuBAND, fMenuBarItemLayout);
...
Bool_t GUIMainFrame::ProcessMessage(Long_t msg, Long_t parm1, Long_t)
{
  Int_t ret;
   ...
   UInt_t board_addr[2];
   board_addr[0] = BAND_BOARD_ADDRESS_1;
   board_addr[1] = 0;
...
void ScalersDlg::ReadVME()
{

  {
	...
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_D4_SCALER_BASE + jj*4, &D4[jj]);	
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_D5_SCALER_BASE + jj*4, &D5[jj]);	
...
void Dsc2Dlg::ReadVME()
{
  ...
  addr[0]  = BAND_DSC2_ADDRESS_SLOT12;
  addr[1]  = BAND_DSC2_ADDRESS_SLOT13;
  addr[2]  = BAND_DSC2_ADDRESS_SLOT14;
  addr[3]  = BAND_DSC2_ADDRESS_SLOT15;
  addr[4]  = BAND_DSC2_ADDRESS_SLOT17;
  addr[5]  = BAND_DSC2_ADDRESS_SLOT18;
  addr[6]  = BAND_DSC2_ADDRESS_SLOT19;
  addr[7]  = BAND_DSC2_ADDRESS_SLOT20;

  {
    /* should do in dsc2Config, here just in case; will move it to constructor, should be called once */
    tmp = 0x0004;
    for(ii=0; ii<NDSC; ii++) tcp->Write32(addr[ii] + BAND_DSC2_SCALER_GATE, &tmp);

    tmp = 0x0000;
    for(ii=0; ii<NDSC; ii++) tcp->Write32(addr[ii] + BAND_DSC2_SCALER_LATCH, &tmp);
    ...
    for(ii=0; ii<NDSC; ii++) tcp->Read32(addr[ii] + BAND_DSC2_SCALER_REF, &ref[ii]);


    for(jj=0; jj<ND; jj++) {tcp->Read32(addr[0] + BAND_DSC2_SCALER_BASE + jj*4,     &D1[jj]); refD1[jj]    = ref[0];}
    for(jj=0; jj<ND; jj++) {tcp->Read32(addr[1] + BAND_DSC2_SCALER_BASE + jj*4,     &D2[jj]); refD2[jj]    = ref[1];}
    for(jj=0; jj<ND; jj++) {tcp->Read32(addr[2] + BAND_DSC2_SCALER_BASE + jj*4,     &D3[jj]); refD3[jj]    = ref[2];}
    for(jj=0; jj<ND; jj++) {tcp->Read32(addr[3] + BAND_DSC2_SCALER_BASE + jj*4,     &D4[jj]); refD4[jj]    = ref[3];}
    for(jj=0; jj<ND; jj++) {tcp->Read32(addr[4] + BAND_DSC2_SCALER_BASE + jj*4,     &D5[jj]); refD5[jj]    = ref[4];}
    for(jj=0; jj<ND; jj++) {tcp->Read32(addr[5] + BAND_DSC2_SCALER_BASE + jj*4,     &D6[jj]); refD6[jj]    = ref[5];}
    for(jj=0; jj<ND; jj++) {tcp->Read32(addr[6] + BAND_DSC2_SCALER_BASE + jj*4,     &D7[jj]); refD7[jj]    = ref[6];}
    for(jj=0; jj<ND; jj++) {tcp->Read32(addr[7] + BAND_DSC2_SCALER_BASE + jj*4,     &D8[jj]); refD8[jj]    = ref[7];}

	
	printf("Scalers: ref1=%d, D1=%d %d %d (address=0x%08x)\n",
      ref[0],D1[0],D1[1],D1[2],addr[0] + BAND_DSC2_SCALER_BASE);
...
void DelaysDlg::ReadVME()
{
...

  {
	...
	/* ERROR BAND_W_DELAY_BASE !!! */
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_U_DELAY_BASE + jj*4, &D1[jj]);
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_V_DELAY_BASE + jj*4, &D2[jj]);
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D3[jj]);
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D4[jj]);
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D5[jj]);
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D6[jj]);
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D7[jj]);
	for(jj=0; jj<ND; jj++) tcp->Read32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D8[jj]);
...
void DelaysDlg::WriteVME()
{
...

  {
    ...
    for(jj=0; jj<ND; jj++) if(D1[jj] != D1GUI[jj]) tcp->Write32(BAND_BOARD_ADDRESS_1 + BAND_U_DELAY_BASE + jj*4, &D1GUI[jj]);
    for(jj=0; jj<ND; jj++) if(D2[jj] != D2GUI[jj]) tcp->Write32(BAND_BOARD_ADDRESS_1 + BAND_V_DELAY_BASE + jj*4, &D2GUI[jj]);
    for(jj=0; jj<ND; jj++) if(D3[jj] != D3GUI[jj]) tcp->Write32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D3GUI[jj]);
    for(jj=0; jj<ND; jj++) if(D4[jj] != D3GUI[jj]) tcp->Write32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D4GUI[jj]);
    for(jj=0; jj<ND; jj++) if(D5[jj] != D3GUI[jj]) tcp->Write32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D5GUI[jj]);
    for(jj=0; jj<ND; jj++) if(D6[jj] != D3GUI[jj]) tcp->Write32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D6GUI[jj]);
    for(jj=0; jj<ND; jj++) if(D7[jj] != D3GUI[jj]) tcp->Write32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D7GUI[jj]);
    for(jj=0; jj<ND; jj++) if(D8[jj] != D3GUI[jj]) tcp->Write32(BAND_BOARD_ADDRESS_1 + BAND_W_DELAY_BASE + jj*4, &D8GUI[jj]);
  }
