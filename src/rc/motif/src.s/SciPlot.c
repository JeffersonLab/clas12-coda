/*----------------------------------------------------------------------------
 *ciPlot	A generalized plotting widget
 *
...
static XtResource resources[] =
{
  ...
  {XtNmonochrome, XtCBoolean, XtRBoolean, sizeof(Boolean),
    offset(Monochrome), XtRImmediate, (XtPointer) True},
...
ItemGetFontGC (SciPlotWidget w, SciPlotItem *item)
{
  ...
  gc = w->plot.dashGC;
  if (w->plot.Monochrome || item->kind.any.color >= w->plot.num_colors)
    color = w->plot.ForegroundColor;
  else
    color = item->kind.any.color;
  XSetForeground(XtDisplay(w), gc, w->plot.colors[color]);
...
static void 
ItemPSDrawAll (SciPlotWidget w, FILE *fd, double yflip)
{
  int i, loopcount;
  SciPlotItem *item;
  int previousfont, previousline, currentfont, currentline;
...
Boolean 
SciPlotPSCreateFancy (SciPlotWidget w, char *filename, int drawborder, char *titles)
{
  FILE *fd;
