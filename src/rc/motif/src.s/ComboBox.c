...
static XtResource resources[] = {
	...
	offset(combobox.SelectionPolicy), XmRImmediate, (XtPointer) XmBROWSE_SELECT
    }, 
    ...
    }, 
    {
    	...
    	offset(combobox.ScrollBarDisplayPolicy), XmRImmediate, (XtPointer) XmAS_NEEDED
    },
    {
        XmNlistSizePolicy, XmCListSizePolicy, XmRListSizePolicy, sizeof(unsigned char),
        offset(combobox.ListSizePolicy), XmRImmediate, (XtPointer) XmVARIABLE
    },
    {
	...
	offset(combobox.Dummy), XmRImmediate, (XtPointer) 0
    }, 
    {
 ...
 */
    ...
    new->combobox.SelectionPolicy = current->combobox.SelectionPolicy;
    new->combobox.ListSizePolicy  = current->combobox.ListSizePolicy;
    new->combobox.StaticList      = current->combobox.StaticList;
 ...
 */
    if ( current->core.sensitive != new->core.sensitive ) {
        XtSetSensitive(new->combobox.ListCtrl, new->core.sensitive);
        XtSetSensitive(new->combobox.EditCtrl, new->core.sensitive);
        XtSetSensitive(new->combobox.ArrowCtrl, new->core.sensitive);
        XtSetSensitive(new->combobox.ListCtrl, new->core.sensitive);
        if ( !new->core.sensitive )
            ShowHideDropDownList(new, NULL, False);
    }
/* 
 * Die ScrollBarPolicy kann nur dann geaendert werden, wenn das Listenfeld
 * dauerhaft dargestellt wird.
 */
    if ( new->combobox.ScrollBarDisplayPolicy != 
           current->combobox.ScrollBarDisplayPolicy )
        if ( new->combobox.StaticList )
            XtVaSetValues(new->combobox.ListCtrl,
                          XmNscrollBarDisplayPolicy, new->combobox.ScrollBarDisplayPolicy,
                          NULL);
        else
            XtWarning(
...
/* Anzahl der in der Liste gleichzeitig darstellbaren Eintraege */
    ...
    if ( current->combobox.AutomaticSelection != 
           new->combobox.AutomaticSelection )
	XtVaSetValues(new->combobox.ListCtrl, 
	              XmNautomaticSelection, new->combobox.AutomaticSelection, 
		      NULL);
 ...
 */
	for ( j = 0; j < MirrorSize; j++ ) {
		...
		case RWI: /* Zugriff auf XmNitems */
			...
			if ( strcmp(args[k].name, XmNitemCount) == 0 ) {
			    ...
			    XtSetValues(new->combobox.ListCtrl,
			                args, 2);
			    /*XtVaSetValues(new->combobox.ListCtrl, 
    ...
    if ( (new->combobox.SquareArrow != current->combobox.SquareArrow) ||
         (new->combobox.ArrowSpacingOn != current->combobox.ArrowSpacingOn) ) {
        Update = False;
        DoLayout(new);
    }
			        ...
			        KeySym *pKeySym)
{
	...
	RscDest.addr = (XtPointer) pKeySym;
	if ( XtConvertAndStore(w, RepresentationType, &RscValue, 
	                       XmRKeySym, &RscDest) )
	    return True;
    }
...
typedef struct {
    ...
    enum { RInt, RShort, RLType, RDimension, 
           RXmString, RPixmap, RXmFontList, RKeySym, RString, 
...
/* Allgemeine Initialisierungen... */
    ...
    new->combobox.LastSelection                    = 0;
    
    new->combobox.InInit                           = True;
...
/* --- */
    ...
    if ( new->combobox.StaticList ) {
	XtVaSetValues(new->combobox.ArrowCtrl, 
 ...
 */
     ...
     */
	new->combobox.PopupShell = (Widget) new;
    }
 ...
 */
    XtSetArg(args[n], XmNselectionPolicy, new->combobox.SelectionPolicy); n++;
    ...
    XtSetArg(args[n], XmNlistSizePolicy, 
                      new->combobox.ListSizePolicy); n++;
    XtSetArg(args[n], XmNscrollBarDisplayPolicy, 
                      new->combobox.ScrollBarDisplayPolicy); n++;
    
    XtSetArg(args[n], XmNautomaticSelection, 
                      new->combobox.AutomaticSelection); n++;
 ...
 */
	...
	if ( !new->combobox.Editable ) {
	    XtVaSetValues(XtParent(new->combobox.ListCtrl), 
		      XmNtraversalOn, False, NULL);
	    XmRemoveTabGroup(new->combobox.ListCtrl);
	}
    }
    ...
    XtAddEventHandler(new->combobox.ListCtrl, 
                      ButtonReleaseMask, 
    ...
    XtOverrideTranslations(new->combobox.ListCtrl, 
                           NewListTranslations);
#endif
    if ( new->combobox.StaticList && new->combobox.Editable )
        XtOverrideTranslations(new->combobox.ListCtrl, 
                               NewListTranslationsE);
