	      ...
	      char* &database, char* &session, char* &msqld)
{
  ...
  while (i < argc)
  {
    if (argv[i][0] == '-')
    {
      ...
      else if (::strcmp (argv[i], "-m") == 0 || ::strcmp (argv[i], "-msqld_host") == 0)
      {
	    i++;
	    if (i >= argc || argv[i][0] == '-')
        {
      ...
      else if (::strcmp (argv[i], "-i") == 0 || ::strcmp (argv[1], "-interactive") == 0)
      {
	    i++;
	    tcl_interactive = 1;
      }
...
main(int argc, char **argv)
{
  ...
  if (database == 0) database = getenv ("EXPID");
  if (session == 0) session = getenv ("SESSION");
  if (msqld == 0) msqld = getenv ("MYSQL_HOST");
  ...
  if (tcl_interactive) rccStdinPipe     rccStdin (reactor);
