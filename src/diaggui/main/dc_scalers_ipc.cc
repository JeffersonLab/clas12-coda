...
void
dc_scalers_app::hist2root(Hist hist)
{
  Int_t id, ibinx, ibiny;
  Float_t x, y, *content;
  char title[20];

  printf("hist2root reached\n");
  ...
  if(hist.nbinx>0)
  {
    ...
    if(hist.nbiny == 0) /* 1-dim */
    {
      ...
      phist1[0]->SetEntries(hist.entries+phist1[0]->GetEntries());
      content = phist1[0]->GetArray();
      ...
      for(ibinx=0; ibinx<hist.nbinx; ibinx++)
	  {
        ...
        phist1[0]->Fill((float)ibinx, (float)hist.buf[ibinx]);
	  }
	  */
    ...
    {
	  /*
      phist2[0]->SetEntries(hist.entries+phist2[0]->GetEntries());
      content = phist2[0]->GetArray();
      content[0] += (float)hist.xunderflow;
      content[hist.nbinx+1] += (float)hist.xoverflow;
      for(ibinx=0; ibinx<hist.nbinx; ibinx++)
	  {
	  ...
	  */
	  ...
	  for(ibinx=0; ibinx<hist.nbinx; ibinx++)
      {
        x = hist.xmin + hist.dx*(float)ibinx;
        for(ibiny=0; ibiny<hist.nbiny; ibiny++)
        {
          y = hist.ymin + hist.dy*(float)ibiny;

          printf("hist2root: 2DIM[%f][%f] => %f\n",x,y,(float)hist.buf2[ibinx][ibiny]); fflush(stdout);
          phist2[0]->Fill(x,y,(float)hist.buf2[ibinx][ibiny]);
	    }
	  }
...
//  pB->Connect("Clicked()", "dc_scalers_app", this, "DoExit()");
  ...
  pCanvas->GetCanvas()->Divide(4,2,0.01,0.01);
  ...
  for(int i=5;i<6;i++)
  {
    phist1[i-5] = new TH1F("test1", "test1", 100, 0.0, 100.0);
    pCanvas->GetCanvas()->cd(i+1);
    phist1[i-5]->Draw("");
  }
  for(int i=6;i<7;i++)
  {
    phist2[i-6] = new TH2F("test2", "test2", 100, 0.0, 100.0, 100, 0.0, 100.0);
    pCanvas->GetCanvas()->cd(i+1);
    phist2[i-6]->Draw("LEGO");
  }
...
main(int argc, char* argv[])
{
  ...
  MessageActionHist *hist = new MessageActionHist((char *)"uthbook_recv", debug, pdc_scalers_app);
  ...
  while(1)
  {
    //pdc_scalers_app->mutex.Lock();
 	gSystem->ProcessEvents();
    //pdc_scalers_app->mutex.UnLock();
