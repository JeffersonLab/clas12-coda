...
/* get_run_events.cc - returns nevents from database */
...
int
get_run_ndata(char *mysql_database, char *configname)
{
  MYSQL *connNum;
  MYSQL_RES *result;
  MYSQL_ROW row_out;
  ...
  query << "SELECT value FROM " << configname << "_option WHERE name='ndata'" << ends;
