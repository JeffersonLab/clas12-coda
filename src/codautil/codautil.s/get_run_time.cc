...
/* get_run_time.cc - returns current run unixtime from database */
...
int
get_run_time(char *mysql_database, char *configname)
{
  MYSQL *connNum;
  MYSQL_RES *result;
  MYSQL_ROW row_out;
  ...
  query << "SELECT value FROM " << configname << "_option WHERE name='unixtime'" << ends;
