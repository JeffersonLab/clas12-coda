...
vmeSetVMEDebugModeOutputFilename(char *fOutput)
{
  ...
  printf("%s: Output set to %s\n",__FUNCTION__,fOutput);

  return OK;
}

int
vmeSetVMEDebugModeOutput(FILE *fOutput)
{
  fDebugMode = fOutput;

  if(fOutput == stdout)
    printf("%s: Output set to STDOUT\n",__FUNCTION__);
  else if(fOutput == stderr)
...
{
#ifdef GEFANUC
  return jlabgefSetA32BltWindowWidth(size);
#else
  return OK;
#endif
}
...
#ifdef GEFANUC
  status = jlabgefDmaSendPhys(physAdrs,vmeAdrs,size);
#endif
