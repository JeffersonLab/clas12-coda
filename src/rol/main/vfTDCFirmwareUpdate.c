/*----------------------------------------------------------------------------*
 *  Copyright (c) 2017        Southeastern Universities Research Association, *
 ...
 *
 * Description:
 *     Firmware update for the Jefferson Lab VXS FPGA-Based
 *      Time to Digital Converter (vfTDC) module
 *
 *----------------------------------------------------------------------------*/
...
/*
  ...
  slot 3: vfTDCFirmwareUpdate 0x180000 vfBtdc29.svf
   ........................................
  slot 19: vfTDCFirmwareUpdate 0x980000 vfBtdc29.svf
  slot 20: vfTDCFirmwareUpdate 0xA00000 vfBtdc29.svf

previous firmware:
  slot 19: vfTDCFirmwareUpdate 0x980000 vfBtdc28.svf
  slot 20: vfTDCFirmwareUpdate 0xA00000 vfBtdc28.svf
 */
...
#endif /*TEMPE*/
#endif
extern volatile struct vfTDC_struct *TDCp[VFTDC_MAX_BOARDS + 1];
extern int vfTDCID[VFTDC_MAX_BOARDS];
...
int
#ifdef VXWORKS
vfTDCFirmwareUpdate(unsigned int arg_vmeAddr, char *arg_filename)
#else
...
#endif
{
  ...
  stat = vfTDCInit(vme_addr,
		   0,
		   1,
		   VFTDC_INIT_SOFT_SYNCRESET |
		   VFTDC_INIT_SOFT_TRIG |
		   VFTDC_INIT_INT_CLKSRC |
		   VFTDC_INIT_SKIP_FIRMWARE_CHECK | VFTDC_INIT_SKIP);
  if (stat != 1) /* expect 1 board to be initialized */
  {
    printf("\n");
    printf("*** Failed to initialize vfTDC ***\nThis may indicate (either):\n");
    printf("   a) an incorrect VME Address provided\n");
    printf("   b)  vfTDC is unresponsive and needs firmware reloaded\n");
    printf("\n");
...
REPEAT:
  ...
  if (badInit == 0)
    {
      firmwareInfo = vfTDCGetFirmwareVersion(0) << 20;
      if (firmwareInfo > 0)
	{
	  printf
	    ("\n  Board Firmware (Version.Revision) = %d.%d\n",
	     (firmwareInfo & VFTDC_STATUS_FIRMWARE_VERS_MASK) >> 24,
	     (firmwareInfo & VFTDC_STATUS_FIRMWARE_REV_MASK) >> 20);
	  printf("\n");
	}
    else
	{
  ...
  printf("Press y to load firmware (%s) to the vfTDC via VME...\n", filename);
...
REPEAT2:
  ...
  /* Check to see if the vfTDC is in a VME-64X crate or Trying to recover corrupted firmware */
  if (badInit == 0)
    geo = vfTDCGetGeoAddress(0);
  else
    geo = -1;
  ...
  if (geo <= 0)
    ...
    {
      laddr = (unsigned long)TDCp[vfTDCID[0]];
    }
