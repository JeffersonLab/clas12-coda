...
int                 vfTDCID[VFTDC_MAX_BOARDS];      /* array of slot numbers for vfTDCs */
unsigned int        vfTDCAddrList[VFTDC_MAX_BOARDS];            /* array of a24 addresses for vfTDCs */
unsigned long       vfTDCA24Offset   = 0;       /* Difference in CPU A24 Base and VME A24 Base */
unsigned int        vfTDCA32Base     = 0x09000000;/*sergey*/   /* Minimum VME A32 Address for use by TI */
 ...
 *         0 1  HFBR1
 *         1 0  VXS
 *         1 1  Internal 125MHz Clock
 * </pre>
 *
 * <pre>
 *      bit 16:  Exit before board initialization
 *             0 Initialize vfTDC (default behavior)
 *             1 Skip initialization (just setup register map pointers)
 *
 *      bit 17:  Use vfTDCAddrList instead of addr and addr_inc
 *               for VME addresses.
...
printf("clkSrc=0x%08x\n",clkSrc);
  ...
  if(addr==0)
    ...
    { /* A24 Addressing */
      printf("%s: ERROR: A32 Addressing not allowed for vfTADC configuration space\n",
	     __FUNCTION__);
      return(ERROR);
    }
  else
  ...
  for (ii=0;ii<ntdc;ii++)
  {
    ...
    else
	{
	  ...
	  else
	  {
        ...
        printf("%s: FIRMWARE = 0x%X\n",__FUNCTION__,fwvers);
	    if(!noFirmwareCheck)
		{
		  /* Check FPGA firmware version */
		  if( fwvers != VFTDC_SUPPORTED_FIRMWARE && fwvers != VFTDC_SUPPORTED_FIRMWARE_OLD )
		  {
		    printf("%s: ERROR: Slot %2d: FPGA Firmware (0x%X) not supported by this driver.\n",
			     __FUNCTION__,boardID, fwvers);
		    printf("\tUpdate to 0x%X to use this driver.\n",VFTDC_SUPPORTED_FIRMWARE);
		    continue;
		  }
	    ...
	    else
		{
		  /* Check FPGA firmware version */
		  if( fwvers != VFTDC_SUPPORTED_FIRMWARE && fwvers != VFTDC_SUPPORTED_FIRMWARE_OLD )
		      printf("%s: WARN: Slot %2d: FPGA Firmware (0x%x) not supported by this driver (ignored).\n",
			     __FUNCTION__,boardID, fwvers);
		}
        ...
        /*sergey*/
        /*for the first board in the group, set vfTDCA32Base based on slot number*/
        if(nvfTDC==0)
		{
          if(slot2a32[boardID]==0) printf("WARN: cannot define vfTDCA32Base for slot %d\n",boardID);
          else
		  {
            vfTDCA32Base = slot2a32[boardID];
            printf("INFO: set vfTDCA32Base = 0x%08x\n",vfTDCA32Base);
		  }
		}  
        /*sergey*/
  ...
  if(!noBoardInit)
  {
	...
	printf("clkSrc(switch)=0x%08x (options=0x%08x 0x%08x 0x%08x 0x%08x)\n",clkSrc,
			 VFTDC_INIT_INT_CLKSRC_125,VFTDC_INIT_INT_CLKSRC_250,VFTDC_INIT_HFBR1_CLKSRC,VFTDC_INIT_VXS_CLKSRC);
    switch(clkSrc)
	{
	case VFTDC_INIT_INT_CLKSRC_125:
	  wreg = VFTDC_CLOCK_INTERNAL_125;
      printf("%s: INFO: Clock Source Internal_125\n",__FUNCTION__);
	  break;

	case VFTDC_INIT_INT_CLKSRC_250:
	  wreg = VFTDC_CLOCK_INTERNAL_250;
      printf("%s: INFO: Clock Source Internal_250\n",__FUNCTION__);
	  break;

	case VFTDC_INIT_HFBR1_CLKSRC:
	  wreg = VFTDC_CLOCK_HFBR1;
      printf("%s: INFO: Clock Source HFBR1\n",__FUNCTION__);
	  break;

	case VFTDC_INIT_VXS_CLKSRC:
	  wreg = VFTDC_CLOCK_VXS;
      printf("%s: INFO: Clock Source VXS\n",__FUNCTION__);
	  break;
	...
	default:
	  ...
	  printf("clkSrc=0x%08x, wreg=0x%08x\n",clkSrc,wreg);
    ...
    for(ii=0;ii<nvfTDC;ii++)
	{
      ...
      printf("CLOCK? (board %d)\n",ii);fflush(stdout);
	  printf("CLOCK=0x%08x\n",(vmeRead32(&TDCp[vfTDCID[ii]]->clock)>>8) & VFTDC_CLOCK_MASK );fflush(stdout);
      printf("CLOCK!\n");fflush(stdout);
	}
    taskDelay(5);
	...
	case VFTDC_INIT_FP_TRIG:
	  wreg = VFTDC_TRIGSRC_FP;
	  break;
  ...
  if(errFlag > 0)
  {
    ...
    if(nvfTDC > 0)
	{
	  printf("%s: %d VFTDC(s) successfully initialized, but errFlag=%d\n",
           __FUNCTION__,nvfTDC,errFlag);
      return(nvfTDC/*ERROR*/);
	}
  }
  else
...
vfTDCStatus(int id, int pflag)
{
  ...
  vr.ptw            = (vmeRead32(&TDCp[id]->ptw))&VFTDC_PTW_MASK; //sergey: add mask
  vr.intsetup       = vmeRead32(&TDCp[id]->intsetup);
  vr.pl             = (vmeRead32(&TDCp[id]->pl))&VFTDC_PL_MASK; //sergey: add mask
  ...
  for(icon = 0; icon < 6; icon++)
    vr.input_enable[icon] = vmeRead32(&TDCp[id]->input_enable[icon]);

  vr.ref_input      = vmeRead32(&TDCp[id]->ref_input);
  ...
  printf(" Board Firmware (Version.Revision) = %d.%d  -- %s\n\n",
	 (vr.status&VFTDC_STATUS_FIRMWARE_VERS_MASK)>>24,
	 (vr.status&VFTDC_STATUS_FIRMWARE_REV_MASK)>>20,
	 board_types[(vr.status & VFTDC_STATUS_BOARD_TYPE_MASK) >> 30]);
  ...
  printf("\n");
  printf("   Connector   Logic Channel Masks\n");

  const char *conn_name[6] =
    {
      " A",
      " B",
      " C",
  ...
  for(icon = 0; icon < 6; icon++)
    {
      printf("       %s             0x%08x\n",
	     conn_name[icon], vr.input_enable[icon]);
    }
  
  printf("\n");
  printf("  Output width = %d (%d ns)\n",
	 (vr.ref_input & VFTDC_REFINPUT_LOGIC_WIDTH_MASK) >> 8,
	 4 * ((vr.ref_input & VFTDC_REFINPUT_LOGIC_WIDTH_MASK) >> 8));

  printf("  Extra Logic Input #5: %s\n",
	 (vr.ref_input & VFTDC_REFINPUT_INPUT5_LOGIC_ENABLE) ? "Enabled" : "Disabled");

  printf("\n");

  printf("\n latency=%d ticks, width=%d ticks\n\n",vr.pl,vr.ptw); //sergey
 ...
 */
int
vfTDCGetFirmwareVersion(int id)
{
  unsigned int rval=0;
  if(id==0) id=vfTDCID[0];
  ...
  VLOCK;
  rval = (vmeRead32(&TDCp[id]->status)&VFTDC_STATUS_FIRMWARE_VERSION_MASK)>>20;
  VUNLOCK;
...
/**
 * @ingroup Config
 * @brief Set the trigger window parameters
 *
 * @param id Slot Number
 ...
 */
int
vfTDCSetWindowParameters(int id, int latency, int width)
{
  if(id==0) id=vfTDCID[0];
...
vfTDCReadBlock(int id, volatile UINT32 *data, int nwrds, int rflag)
{
  ...
  { /* Block Transfers */
	...
	//printf("vfTDCReadBlock: vfTDCA32Offset=0x%08x vmeAdr=0x%08x laddr=0x%08x nwrds=%d\n",vfTDCA32Offset,vmeAdr,laddr,nwrds);fflush(stdout);
...
*/
	...
	//printf("vfTDCReadBlock: retVal(usrVme2MemDmaStart)=%d\n",retVal);fflush(stdout);
...
*/
	...
	//printf("vfTDCReadBlock: retVal(usrVme2MemDmaDone)=%d\n",retVal);fflush(stdout);
    ...
    if(retVal > 0)
	{
	  ...
	  //printf("vfTDCReadBlock: stat=%d (csr=0x%08x\n",stat,csr);
  ...
  {  /*Programmed IO */
    ...
    if(berr)
	{
      printf("ERROR: berr(1)=0x%08x\n",berr);
      vmeWrite32(&TDCp[id]->vmeControl, vmeRead32(&TDCp[id]->vmeControl) & ~VFTDC_VMECONTROL_BERR);
	}
    ...
    if((bhead&VFTDC_DATA_TYPE_DEFINE)&&((bhead&VFTDC_DATA_TYPE_MASK) == VFTDC_DATA_BLOCK_HEADER))
	{
	  ...
	  ehead = LSWAP(ehead);
#endif
      printf("bhead=0x%08x ehead=0x%08x\n",bhead,ehead);
    ...
    else
	{
	  ...
	  if( ((vmeRead32(&TDCp[id]->blockBuffer) &
			VFTDC_BLOCKBUFFER_BLOCKS_READY_MASK)>>16) == 0) /*sergey: must be '>>8' ??? */
	  {
	    printf("vfTDCReadBlock: FIFO Empty (0x%08x)\n",bhead);
	    VUNLOCK
	    return(0);
	  }
	  else
...
vfTDCBReady(int id)
{
  ...
  //printf("vfTDCBReady(%2d): rval=%d blockBuffer=0x%08x (0x%08x)\n",id,rval,blockBuffer,vmeRead32(&TDCp[id]->blank1[0]));
 ...
 * @param   source
 *         -   0:  HFBR1
 *         -   1:  Internal clock (250 MHz)
 *         -   2:  Internal clock (125 MHz)
 *         -   3:  VXS
 *
 * @return OK if successful, otherwise ERROR
 */
...
vfTDCSetClockSource(int id, unsigned int source)
{
  ...
  switch(source)
    {
    case VFTDC_CLOCK_INTERNAL_250:
      sprintf(sClock,"ONBOARD 250 MHz (%d)",source);
      break;
    case VFTDC_CLOCK_INTERNAL_125:
      sprintf(sClock,"ONBOARD 125 MHz (%d)",source);
      break;
  ...
  if(source==1) /* Turn on running mode for External Clock verification */
    {
      ...
      taskDelay(1);
      clkread = ((vmeRead32(&TDCp[id]->clock))>>8) & VFTDC_CLOCK_MASK;
      if(clkread != clkset)
	{
...
vfTDCGetClockSource(int id)
{
  ...
  VLOCK;
  rval = ((vmeRead32(&TDCp[id]->clock))>>8) & VFTDC_CLOCK_MASK;
  VUNLOCK;

  printf("vfTDCGetClockSource[slot=%d]: CLOCK=0x%08x\n",id,rval);
...
/**
 * @ingroup Config
 * @brief Enable specified channel mask for specified input connector
 *        in the trigger logic
 * @param id Slot Number
 * @param conn Connector input [0,5] 0:=A ... 4:= VA, 5:=VC
 * @param chanmask Channel mask [0, 0xFFFFFFFF]
 * @return OK if successful, otherwise ERROR
 */
int
vfTDCSetLogicInputChannelMask(int id, int conn, unsigned int chanmask)
{
  if(id==0) id=vfTDCID[0];
  ...
  if((conn < 0) || (conn > 5))
    {
      printf("%s: ERROR: Invalid specified connector (conn = %d)\n",
	     __func__, conn);
      return ERROR;
    }

  VLOCK;
  vmeWrite32(&TDCp[id]->input_enable[conn], chanmask);
  VUNLOCK;
 ...
 */
int
vfTDCSetLogicRefChannelMask(int id, int conn, unsigned int chanmask)
{
  if(id==0) id=vfTDCID[0];
...
/**
 * @ingroup Status
 * @brief Get specified channel mask for specified input connector
 *        in the trigger logic
 * @param id Slot Number
 * @param conn Connector input [0,5] 0:=A ... 4:= VA, 5:=VC
 ...
 */
unsigned int
vfTDCGetLogicRefChannelMask(int id, int conn)
{
  unsigned int rval = 0;
  if(id==0) id=vfTDCID[0];
  ...
  VLOCK;
  rval = vmeRead32(&TDCp[id]->input_enable[conn]);
  VUNLOCK;
...
/**
 * @ingroup Config
 * @brief Set the OR output width (in steps of 4ns) of output #8
 * @param id Slot Number
 * @param width Output width in steps of 4ns [0, 0xFF]
 * @return OK if successful, otherwise ERROR
 */

int
vfTDCSetLogicOutputWidth(int id, unsigned int width)
{
  if(id==0) id=vfTDCID[0];
  ...
  VLOCK;
  vmeWrite32(&TDCp[id]->ref_input,
	     (vmeRead32(&TDCp[id]->ref_input) & ~VFTDC_REFINPUT_LOGIC_WIDTH_MASK) |
	     (width << 8));
  VUNLOCK;
...
/**
 * @ingroup Status
 * @brief Get the OR output width (in steps of 4ns) of output #8
 * @param id Slot Number
 * @return OK if successful, otherwise ERROR
 */

int
vfTDCGetLogicOutputWidth(int id)
{
  int rval = 0;
  if(id==0) id=vfTDCID[0];
  ...
  VLOCK;
  rval = (vmeRead32(&TDCp[id]->ref_input) & VFTDC_REFINPUT_LOGIC_WIDTH_MASK) >> 8;
  VUNLOCK;
...
/**
 * @ingroup Config
 * @brief Set input #5 as an extra LOGIC OR
 * @param id Slot Number
 * @param enable Enable flag [0, 1]
 * @return OK if successful, otherwise ERROR
 */

int
vfTDCSetExtraLogicInput(int id, int enable)
{
  if(id==0) id=vfTDCID[0];
  ...
  if(enable)
    vmeWrite32(&TDCp[id]->ref_input,
	       vmeRead32(&TDCp[id]->ref_input) | VFTDC_REFINPUT_INPUT5_LOGIC_ENABLE);
  else
    vmeWrite32(&TDCp[id]->ref_input,
	       vmeRead32(&TDCp[id]->ref_input) & ~VFTDC_REFINPUT_INPUT5_LOGIC_ENABLE);
  VUNLOCK;
...
/**
 * @ingroup Config
 * @brief Get status of input #5 as an extra LOGIC OR
 * @param id Slot Number
 * @return 1 if enabled, 0 if disabled, otherwise ERROR
 */

int
vfTDCGetExtraLogicInput(int id)
{
  int rval = 0;
  if(id==0) id=vfTDCID[0];
  ...
  VLOCK;
  rval = (vmeRead32(&TDCp[id]->ref_input) & VFTDC_REFINPUT_INPUT5_LOGIC_ENABLE) >> 4;
  VUNLOCK;
...
/*sergey*/
int
vfTDCSlot(unsigned int i)
{
  if(i>=nvfTDC)
    {
      printf("%s: ERROR: Index (%d) >= vfTDCs initialized (%d).\n",
	     __FUNCTION__,i,nvfTDC);
      return ERROR;
    }
...
void
vfTDCSetA32BaseAddress(unsigned int addr)
{
  vfTDCA32Base = addr;
  printf("vfTDC A32 base address set to 0x%08X\n",vfTDCA32Base);
}
