...
#include <unistd.h>
#include <stddef.h>
#include <stdint.h> /*sergey: for uint32_t etc*/
#endif
#include <pthread.h>
#include <stdio.h>
#include "jvme.h"
...
hdInit(uint32_t vAddr, uint8_t source, uint8_t helSignalSrc, uint32_t iFlag)
{
  ...
  if(noBoardInit)
    {
      //sergey: in hallb it suppose to return the number of boards found
      //return OK;
      return(1);
    }
...
hdReadBlock(volatile unsigned int *data, int nwrds, int rflag)
{
  ...
  if(rflag >= 1)
      ...
      else
	{
      ...
      */
      retVal = usrVme2MemDmaStart(vmeAdr, /*(unsigned long)*/(devaddr_t)laddr, (nwrds<<2));
...
hdSetTSettleFilter(uint8_t clock)
{
  ...
  HLOCK;
  vmeWrite32(&hdp->ctrl1,
	     ((vmeRead32(&hdp->ctrl1) &~ HD_CTRL1_TSETTLE_FILTER_MASK) | (clock<<13) ));
  HUNLOCK;
