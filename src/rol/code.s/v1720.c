...
unsigned int c1720vme[V1720_MAX_MODULES]; /* jumper addresses for Dma */
...
v1720Config(int options)
{
  ...
  */
  ...
  for(ii=0; ii<Nc1720; ii++)
  {
    ...
    v1720SetTriggerEnableMask(ii, 0, 1, 0, 0);
...
v1720ReadBoardDmaStart(int ib, UINT32 *tdata)
{
  ...
  {
    /* get event length in words */
    ndata_save = v1720GetNextEventSize(ib);
    /*
  ...
  if(berr_fifo == 0x01) /* FIFO readout */
  {
	...
	*/
    ...
    nbytes = ((ndata_save+extra_save)<<2);
  }
  else
  {
    nbytes = (ndata_save<<2);
