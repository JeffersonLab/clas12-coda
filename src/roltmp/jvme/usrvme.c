...
void
usrChangeVmeDmaMemory(UINT32 pMemBase, UINT32 uMemBase, UINT32 mSize)
{
  physMemBase = pMemBase;
  userMemBase = uMemBase;
  memSize = mSize;
...
{
  physMemBase = physMemBaseSave;
  userMemBase = userMemBaseSave;
  memSize = memSizeSave;
...
printf("usrVme2MemDmaStart: physMemBase = 0x%08x   offset      = 0x%08x\n\n",physMemBase,offset);
  */
  ...
  if(nbytes<=0/* || nbytes>(memSize-offset) - sergey: DO IT DIFFERENTLY !!!*/) 
  {
