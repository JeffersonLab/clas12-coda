...
/* defined in et_private.h already
#define CODA_ERROR 1
#define CODA_OK 0
*/
...
typedef struct eb_thread_args *ebArg;
typedef struct eb_thread_args
{
  objClass object;
  int *interp_obsolete;
  int *thread_exit;
...
printf("!!!coda_eb: data=0x%08x\n",data);
      ...
      if( (typ < EV_SYNC) || (typ >= (EV_SYNC+16)) ) /* physics event */
      {
		...
		/*
printf("!!!coda_eb: roc=%d desc1->evnb=%d\n",roc,desc1->evnb);
		*/
      }
...
output_event:
    ...
    if(et_reinit == 0 && ebp->end_event_done == 0)
    {
	  ...
	  /*
	  if(desc2.user[3]) printf("coda_eb: syncFlag detected (0x%08x), desc2.type = 0x%08x\n",
...
roc_queue_ix = 0;
  ...
  for(ix=0; ix<linkArgc; ix++)
  {
	...
	if(ebp->links[ix]==NULL)
	{
      printf(">>>>> ERROR opening link from >%s< - exit\n",linkArgv[ix]);
      exit(0);
	}
	else
	{
      printf(">>>>> opened link from >%s< link=0x%08x\n",linkArgv[ix],ebp->links[ix]);
	}
  }
...
/* called at 'End' transition; building thread must receive 'end'
